...
public:
	...
	public:
		explicit
			CIndenter(const std::string& ind = "\t")
...
public:
	...
	{
		return pnode && pnode->type == GUMBO_NODE_ELEMENT ? pnode->v.element.attributes.length : 0;
	}
	...
	inline static GumboAttribute* nthAttr(const GumboNode * pnode, int i)
	{
		if (i >= 0 && i < int(attrNum(pnode))) {
			return reinterpret_cast<GumboAttribute*>(pnode->v.element.attributes.data[i]);
		}
		else {
			return 0;
		}
...
public:
	static void writeInnerHtml(std::ostream& o, GumboNode * apNode, const char * ind = "\t");
	static void writeOuterHtml(std::ostream& o, GumboNode * apNode, const char * ind = "\t");

private:

	static void writeInnerHtml_impl(std::ostream& o, GumboNode * apNode, CIndenter& ind);
	static void writeOuterHtml_impl(std::ostream& o, GumboNode * apNode, CIndenter& ind);
...
inline std::ostream& operator << (std::ostream& o, const CQueryUtil::CIndenter& ind)
{
	ind.print(o);
	return o;
}
